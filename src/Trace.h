...
#include <windows.h>
#include <string.h>
#define QPIFILE ( ( strrchr(__FILE__, '\\') != NULL ) ? strrchr(__FILE__, '\\') + 1 : __FILE__ )
#else
...
//#define DEBUG_GLOBAL( expr ) { stringstream *_ostrlog = new stringstream(); *_ostrlog << "DEBUG [" << QPIFILE << "] - " << __LINE__ << " : " << expr << endl << flush; cerr.write( _ostrlog->str().c_str(), _ostrlog->str().length() ); cerr << flush; delete _ostrlog; }
#define	DEBUG_GLOBAL( expr ) { if ( !FileOutputter::wasTerminated() ) { FileOutputter::getStream() << "DEBUG [" << QPIFILE << "] - " << __LINE__ << " : " << expr << endl << flush; } }
...
#ifdef WIN32_SERVICE
#define TRACE_SERVICE( expr ) { stringstream _errormessage; _errormessage << expr; OutputDebugString( TEXT( _errormessage.str().c_str() ) ); FileOutputter::getStream() << "TRACE [" << QPIFILE << "] - " << __LINE__ << " : " << expr << endl << flush; }
...
#define TRACE_NOLOG( expr ) \
	{ \
		try \
		{ \
			stringstream _errormessage; _errormessage << expr; OutputDebugString( TEXT( _errormessage.str().c_str() ) ); FileOutputter::getStream() << "TRACE [" << QPIFILE << "] - " << __LINE__ << " : " << expr << endl << flush; \
		} \
		catch( ... ){} \
	}
#define DEBUG_NOLOG( expr ) { cout << "DEBUG [" << QPIFILE << "] - " << __LINE__ << " : " << expr << endl << flush; }
#else
#define TRACE_GLOBAL( expr ) { if ( !FileOutputter::wasTerminated() ) { FileOutputter::getStream() << "TRACE [" << QPIFILE << "] - " << __LINE__ << " : " << expr << endl << flush; } }
...
#define TRACE_NOLOG( expr ) \
	{ \
		try \
		{ \
			FileOutputter::getStream() << "TRACE [" << QPIFILE << "] - " << __LINE__ << " : " << expr << endl << flush; \
		} \
		catch( ... ){} \
		try \
		{ \
			cerr << "TRACE [" << QPIFILE << "] - " << __LINE__ << " : " << expr << endl << flush; \
		} catch( ... ){}; \
	}
#define DEBUG_NOLOG( expr ) { cout << "DEBUG [" << QPIFILE << "] - " << __LINE__ << " : " << expr << endl << flush; }
#endif // WIN32_SERVICE
